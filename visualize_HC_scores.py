...
LIST_OF_COLORS  = ["#F8766D","#00BA38","#619CFF", 'tab:red','tab:olive','tab:blue', 'tab:gray', 'tab:orange','tab:purple', 'tab:brown',
                   'tab:pink', 'tab:green', 'tab:cyan', 'royalblue', 'darksaltgray', 'forestgreen', 'cyan', 'navy'
...
def plot_author_pair_HC(df, wrt_author, show_legend = True, title = "") :
    ...
    p = (ggplot(aes(x = 'x',y = 'y', color = 'author'), data = df1) +
       ...
       # geom_text(aes(label = 'doc_id', check_overlap = True)) + 
        ...
        ggtitle(title) + theme(legend_title = element_blank(), legend_position = 'top')
        )
    return p
...
def plot_author_pair_HC_label(df, wrt_author, show_legend = True, title = "") :
    ...
    p = (ggplot(aes(x = 'x',y = 'y', fill = 'author'), data = df1) +
        geom_point(show_legend = show_legend) + geom_abline(alpha = 0.5) +
        geom_label(aes(label = 'doc_id'), size = 10) + 
        xlab(wrt_author[0]) + ylab(wrt_author[1]) +
...
def plot_author_pair_rank(df, wrt_author = ('Author1', 'Author2')) :
    ...
    df1.loc[:,'y'] = df1.loc[:,wrt_author[1]].astype('float')
    p = (ggplot(aes(x = 'x',y = 'y', color = 'author'), data = df1) +
        geom_point(show_legend=False) + geom_abline(alpha = 0.5) + xlim(0,1) + ylim(0,1) +
        xlab(wrt_author[0]) + ylab(wrt_author[1]) +
        scale_color_manual(values = {wrt_author[0] : "red", wrt_author[1] : "blue", 'disputed' : 'black'}) +
...
def plot_author_pair_ROC(df, wrt_author = ('Author1', 'Author2')) :
    ...
    def ROC(seq1, seq2) :
        ...
        roc = pd.DataFrame({'t' : t,
                            'FDR' : np.mean(np.expand_dims(seq1,0) < np.expand_dims(t,1),1),
                            'TDR' : np.mean(np.expand_dims(seq2,0) < np.expand_dims(t,1),1)
                           })    
        return roc
    ...
    p = (ggplot(row, aes(x = 'FDR', y = 'TDR'))
     + geom_line(color = 'red')) # + ggtitle('ROC')
    return p
...
def plot_author_pair_pval(df, wrt_author = ('Author1', 'Author2')) :    
    ...
    df1.loc[:,'pval_x'] = 1 - df1.loc[:,wrt_author[0]].astype('float') * kx / (kx+1)
    ky = df[df.author == wrt_author[1]].doc_id.unique().size
    df1.loc[:,'pval_y'] = 1 - df1.loc[:,wrt_author[1]].astype('float') * ky / (ky+1)

    p = (ggplot(aes(x = 'pval_x',y = 'pval_y', color = 'author'), data = df1) +
        geom_point(show_legend=True) + geom_abline(alpha = 0.5) + xlim(0,1) + ylim(0,1) +
        xlab(wrt_author[0]) + ylab(wrt_author[1]) +
        scale_color_manual(values = {wrt_author[0] : "red", wrt_author[1] : "blue", 'disputed' : 'black'}) +
         theme(legend_title = element_blank(), legend_position = 'top')
        ) # +
...
def plot_author_pair_pval_col(df, wrt_author = ('Author1', 'Author2')) :    
    ...
    df2 = df1.melt(['author','doc_id'],['pval_x', 'pval_y'], var_name = 'wrt_author')
    df2.wrt_author = df2.wrt_author.str.replace('pval_x',wrt_author[0]).replace('pval_y',wrt_author[1])
    df2.doc_id = df2.doc_id.astype(int).astype(str)

    p = (ggplot(aes(x = 'doc_id', y = 'value', fill = 'wrt_author'), data = df2[df2.author == 'disputed']) +
        geom_bar(position='dodge', stat = "identity", show_legend=True) +
        xlab('Document ID') +  ylab('p-value') + scale_fill_manual(['red', 'blue']) +
        theme(legend_title = element_blank(), legend_position = 'top')
        )
        #ggtitle('Rank wrt each author ' + labels[0] + ' vs '+ labels[1])
    return p
