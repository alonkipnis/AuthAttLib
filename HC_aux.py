...
def hc_vals_full(pv, alpha = 0.45):
    ...
    import pandas as pd
    df = pd.DataFrame({'pval' : ps, 'z' : z, 'z_stbl' : z_stbl,
     'u' : uu, 'HC' : hc_star, 'HC_stbl' : hc_star_stbl,
      'p_star' : p_star, 'p_star_stbl' : p_star_stbl})
    return df
...
# Output: data frame: "X, Y, T1, n2, T2, pval, pval_z, hc"
    ...
    counts['pval'] = counts.apply(lambda row: pval_bin(row['n1'], 
                                                         row['n2'], 
                                                         row['T1'],
                                                        row['T2'], 
                                                        min_counts = min_counts), axis=1)
    counts['z'] = counts.apply(lambda row: z_score(row['n1'],
                                                   row['n2'],
                                                   row['T1'],
                                                   row['T2'] 
                                                   ), axis=1)
...
def two_list_test(term_cnt1, term_cnt2, lo_terms = pd.DataFrame(), alpha = 0.35, min_counts = 3):
...
def two_counts_pvals_df(counts_df, min_counts = 1):    
    ...
    counts_df['pval'] = counts_df.apply(lambda row: pval_bin(row['n1'], 
                                                         row['n2'], 
                                                         row['T1'],
                                                        row['T2'], 
                                                        min_counts = min_counts), axis=1)
    counts_df['z'] = counts_df.apply(lambda row: z_score(row['n1'],
                                                   row['n2'],
                                                   row['T1'],
                                                   row['T2']), axis=1)
    return counts_df
